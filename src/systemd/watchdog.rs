...
impl WatchdogManager {
    pub async fn init() -> Option<Interval> {
        ...
        } else if let Some(timeout) = sd_notify::watchdog_enabled() {
            info!(
                "watchdog is enabled with interval: {}µs",
                timeout.as_micros()
            );
    ...
    pub async fn notify() -> Result<()> {
        ...
        if let Some(settings) = settings.as_mut() {
            ...
            sd_notify::notify(&[NotifyState::Watchdog]).context("failed to notify")?;
    ...
    pub async fn interval(timeout: Duration) -> Result<Option<Duration>> {
        ...
        // notify new interval
        sd_notify::notify(&[NotifyState::WatchdogUsec(
