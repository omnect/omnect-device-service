...
impl Feature for FactoryReset {
    ...
    async fn connect_web_service(&self) -> Result<()> {
        web_service::publish(
            ...
            serde_json::to_value(&self.report).context("context")?,
        )
        .await;
...
impl FactoryReset {
    ...
    fn factory_reset_keys() -> Result<Vec<String>> {
        let factory_reset_config: HashMap<String, Vec<std::path::PathBuf>> = from_reader(
            BufReader::new(File::open(config_path!()).context("open factory-reset.json")?),
        )
    ...
    fn factory_reset_result() -> Result<Option<FactoryResetResult>> {
        let path = result_path!().to_path_buf();

        let omnect_os_initramfs_json: serde_json::Value = from_reader(
