...
macro_rules! update_folder_path {
    () => {{
        std::env::var("UPDATE_FOLDER_PATH")
...
macro_rules! du_config_path {
    () => {{
        std::env::var("DEVICE_UPDATE_PATH").unwrap_or("/etc/adu/du-config.json".to_string())
    }};
}

macro_rules! log_file_path {
    () => {{
        std::env::var("SWUPDATE_LOG_PATH").unwrap_or("/var/log/aduc-logs/swupdate.log".to_string())
    }};
}
...
macro_rules! pubkey_file_path {
    () => {{
        std::env::var("SWUPDATE_PUBKEY_PATH")
            .unwrap_or("/usr/share/swupdate/public.pem".to_string())
    }};
}
...
macro_rules! bootloader_updated_file_path {
    () => {{
        std::env::var("BOOTLOADER_UPDATE_PATH")
            .unwrap_or("/tmp/omnect-bootloader-update".to_string())
    }};
}
