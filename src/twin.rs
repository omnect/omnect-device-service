...
pub fn update(
    ...
    debug!("received unhandeld desired twin proberties");
}

pub fn factory_reset_status(tx_app2client: Arc<Mutex<Sender<Message>>>) {
    ...
    if vec.len() == 2 {
        ...
        let state = FactoryResetStatus {
            factory_reset_status: Status {
                status: reset_state,
                date: chrono::offset::Utc::now().to_string(),
            },
        };
